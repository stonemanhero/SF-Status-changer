...
def prepare_mapping():
    ...
    """
    ids_map = {}
    with open('input/fajl_ID_STATUS.csv', 'rb') as f:
        reader = csv.DictReader(f)
        for row in reader:
            ids_map[row['ID'].strip()] = row['DISPOSITION_TYPE__C'].strip()
...
def main():
    ...
    with open('output/changed_dispostition_status.csv', 'wb') as file_output:
        with open('input/svi_novi_leadovi.csv', 'rb') as file_input:
            ...
            for row in reader_input:
                if row['OLD_ORG_ID__C'] in ids_map:
                    row['STATUS__C'] = ids_map[row['OLD_ORG_ID__C']]
